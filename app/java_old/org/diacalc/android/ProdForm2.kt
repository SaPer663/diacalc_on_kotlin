...
class ProdForm : ListActivity() {
    ...
    private var i_prods: ArrayList<ProductInBase>? = null
    private var i_group: ArrayList<ProductGroup>? = null
    private var i_msg = ""
    ...
    private fun setGroupName(sel: Int) {
        if (groups!!.isEmpty()) {
            ...
            groupSelected = sel
            groupTitle!!.text = "" + (groupSelected + 1) + ". " +
                    groups!![groupSelected].name
        }
    }
    ...
    private fun downloadProducts() {
        val t: Thread = object : Thread() {
            override fun run() {
                ...
                i_group = null
                i_msg = ""
                val i_answer = DoingPost(user).requestProducts()
                if (i_answer[DoingPost.ERROR] != null) {
                    //Значит ошибка
                    i_msg = (i_answer[DoingPost.ERROR] as String?)!!
                } else {
                    i_prods = i_answer[DoingPost.PRODS_PRODS] as ArrayList<ProductInBase>?
                    i_group = i_answer[DoingPost.PRODS_GROUP] as ArrayList<ProductGroup>?
